...
import alexrnov.scollection.utils.loader.ModelData;
import alexrnov.scollection.utils.loader.OBJFileLoader;
import alexrnov.scollection.view.ChangeCameraView3D;

import static alexrnov.scollection.utils.commonGL.Buffers.floatBuffer;
import static alexrnov.scollection.utils.commonGL.Buffers.intBuffer;


//import android.util.Log;
//import static alexrnov.cosmichunter.Initialization.TAG;

public abstract class BackgroundObject3D {
  ...
  public BackgroundObject3D(Context context, float scale, String assetFileName) {
    try {
  ...
  public BackgroundObject3D(Context context, float scale, int resourceID) {
   ...
   * методом draw().
   */
  public abstract void setView(ChangeCameraView3D view);

  /**
   * Геттер для объекта, который содержит матрицы для отображения
   * трехмерного объекта в пространстве
   */
  public abstract ChangeCameraView3D getView();
